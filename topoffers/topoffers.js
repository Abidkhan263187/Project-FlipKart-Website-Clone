...
var url = `https://myjson.onrender.com/topOffers` + `?_page=1&_limit=8`
...
function display(productsArray) {
    document.getElementById("abidtopOfferContainer").textContent = ""
    productsArray.forEach(
        ({
            ...
            image.addEventListener("click", () => {
                ...
                localStorage.setItem("prod-Data", "topOffers?id=" + id)
            })
            ...
            Pname.setAttribute("class", "topName")
            Pname = displayName;
            ...
            cardDiv.append(image, detailDiv1, divDetails2, btn);

            document.getElementById("abidtopOfferContainer").append(cardDiv)
...
async function gotoCart(id) {
    try {
        let fetchRes = await geturl(`https://myjson.onrender.com/topOffers` + `/${id}`)
...
function brand() {
    ...
    fetchProducts();
    url = `https://myjson.onrender.com/topOffers`
